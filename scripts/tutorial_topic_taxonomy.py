...
TOPIC_RULES = (
    ...
    TopicRule(
        "多模态",
        r"\b(multimodal|multi.modal|cross.modal|vlm|mllm|audio.language|vision.language)\b|多模态|跨模态|视觉语言模型",
    ),
    TopicRule(
        "计算机视觉",
        r"\b(computer.vision|vision|visual|image|video|ocr|diffusion|segmentation|detection)\b|计算机视觉|视觉|图像|视频|目标检测|分割",
    ),
    TopicRule(
        "模型训练",
...
LEGACY_TOPIC_OPTIONS = {
    ...
    "多模态与视觉": (
        ("多模态", RULES_BY_NAME["多模态"].pattern),
        ("计算机视觉", RULES_BY_NAME["计算机视觉"].pattern),
    ),
    "具身智能与机器人": (("具身智能", None),),
    "AI安全与评测": (
