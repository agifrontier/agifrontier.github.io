...
*   **向量**：d维欧氏空间 $$R^d$$ 中的向量用小写字母 $$x$$ 表示，其分量为 $$x^(i)$$。向量范数采用 $$\lvert x \rvert$$ 表示欧氏范数，$$\lvert x \rvert_p$$ 表示 $$ℓ_p$$ 范数。
*   **矩阵**：$$m x d$$ 实数矩阵的空间记为 $$M_{m,d}(R)$$。算子范数定义为 $$\lvert A \rvert_op = max{\lvert Ax \rvert : \lvert x \rvert = 1}$$。$$S_d$$ 表示对称矩阵空间，$$S_d^+$$ 和 $$S_d^{++}$$ 分别表示半正定和正定矩阵集合。
*   **多线性映射**：定义了k-线性映射及其范数。
...
*   **重要示例**：计算了行列式 $$det(A)$$ 和矩阵逆 $$A^-1$$ 的微分。
    *   $$log\vert{}det(A)\vert{}$$ 的微分是 $$H ↦ trace(A^{-1}H)$$。
...
*   **条件概率与期望**：回顾了离散和连续随机变量的条件概率和条件期望 $$E(Y \vert{} X)$$ 的定义。条件期望满足两个核心性质：
    1.  $$E(Y \vert{} X)(ω)$$ 的值仅依赖于 $$X(ω)$$。
    2.  对于任意合适的函数 $$f$$，有 $$E(E(Y \vert{} X)f(X)) = E(Y f(X))$$。
...
*   **条件概率 (一般情况)**：将条件概率 $$P(Y ∈ A \vert{} X)$$ 定义为指示函数 $$1_{Y∈A}$$ 的条件期望。
