...
- **ArXiv URL**: https://arxiv.org/abs/2509.12168v1
...
当引入外部参考文本 $$R$$（即少样本范例）进行条件生成时，这个概率分布会改变。如果 $$R$$ 中包含了新颖的概念 $$z^*$$（即不在原始训练数据 $$\mathcal{Z}$$ 中的知识），模型的输出概率 $$p(x_i \vert{} R, x_{1:i-1})$$ 会更受 $$z^*$$ 的影响。
