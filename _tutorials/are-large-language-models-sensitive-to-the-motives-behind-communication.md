...
*   **实验设置**：在金融、房地产和医疗等场景中，让 LLM 评估来自不同可信度（角色）和激励水平的说话者所给出的产品推荐。通过提示，引出 LLM 对说话者的“信任分数”（对应 $$λ$$）、“激励分数”（对应 $$R_S$$）和“影响分数”（对应 $$P(R_L\vert{}u)$$），并将其“影响分数”与基于其自身信任和激励分数的理性模型预测值进行比较。
