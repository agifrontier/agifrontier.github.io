...
*   **聊天模板 (Chat Template)**：一种用于格式化对话输入的特定结构，它使用特殊Token（如 $$<\vert{}user\vert{}>$$、$$<\vert{}assistant\vert{}>$$）来包裹用户、助手或系统的消息。本文攻击方法的关键在于，这些模板通常仅在对齐所发生的后训练（post-training）阶段引入。
...
2.  **诱导模型生成**：利用模型后训练阶段引入的聊天模板特殊Token（例如 $$<\vert{}user\vert{}>$$）作为前缀（prefix），重复提示目标模型生成大量样本。
...
下表展示了使用完整聊天模板前缀（$$<\vert{}im_start\vert{}>user\n$$）比仅使用单个特殊Token（$$<\vert{}im_start\vert{}>$$）能生成与后训练数据语义更接近的样本，其平均嵌入分数更高。


| 前缀 | 平均嵌入分数 |
| --- | --- |
| $$<\vert{}im_start\vert{}>$$ | 0.857 |
| $$<\vert{}im_start\vert{}>user\n$$ | 0.892 |
