...
- **ArXiv URL**: https://arxiv.org/abs/2511.03114v1
...
*   **平均分类器 (Mean Classifier)**：为了便于理论分析，本文采用的一种线性分类器。其权重向量 $$w_y$$ 直接使用该类别所有样本在表示空间中的平均表示 $$μ_y$$，即 $$w_y = μ_y = E_{p(x\vert{}y)}[f(x)]$$。下游任务的性能通过该分类器在交叉熵损失下的表现来衡量。
