...
*   **可学习的偏置项 (learnable bias $$b$$)**: 在 Sigmoid 损失中引入的一个额外可学习参数。由于一个批次中负样本对的数量远超正样本对 ($$\lvert B \rvert^2 - \lvert B \rvert$$ vs $$\lvert B \rvert$$)，存在严重的类别不平衡。引入偏置项 $$b$$ 能够缓解此问题，确保训练在初始阶段更加稳定，避免因不平衡导致的剧烈梯度更新。
...
2.  **高内存与计算开销**: 在分布式训练中，计算全局归一化需要昂贵的 $$all-gather$$ 操作来收集所有设备的嵌入向量，并且需要实例化一个大小为 $$\lvert B \rvert × \lvert B \rvert$$ 的相似度矩阵，这极大地限制了批次大小的扩展。
...
### Softmax 损失回顾
对于一个包含 $$\lvert B \rvert$$ 个图文对的批次，传统的对比学习目标是拉近匹配对 $$(I_i, T_i)$$ 的嵌入表示，同时推开不匹配对 $$(I_i, T_j≠i)$$ 的嵌入表示。使用 Softmax 损失函数，其目标函数如下：
...
该实现的核心思想是避免一次性计算和存储整个 $$\lvert B \rvert × \lvert B \rvert$$ 的相似度矩阵。具体步骤如下：
...
这种方式的优势在于：
*   **内存高效**: 在任何时刻，每个设备上只需要存储一个大小为 $$b × b$$（$$b$$ 为单设备批次大小）的小块矩阵，而不是 $$\lvert B \rvert × \lvert B \rvert$$ 的全局矩阵。
