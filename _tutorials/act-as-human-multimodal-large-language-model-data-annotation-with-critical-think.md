...
为此，研究团队从理论上分析并提出了一种**修正后的损失函数** ${\mathcal{L}}^{(ACT)}$。




{% raw %}$$ {\mathcal{L}}^{(ACT)}\_{\mathbf{\theta}}=\frac{1}{N}\sum\_{i=1}^{N}\left(\ell^{(m)}\_{{\mathbf{\theta}},i}+\left(\ell\_{{\mathbf{\theta}},i}-\ell^{(m)}\_{{\mathbf{\theta}},i}\right)\frac{\delta\_{i}(B)}{\pi\_{B}(\hat{\epsilon}\_{i})}\right) $${% endraw %}
