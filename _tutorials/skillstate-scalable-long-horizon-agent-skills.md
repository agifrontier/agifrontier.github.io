...
这种设计使得单步 Prompt 大小被限制在 $\lvert P_t \rvert = \mathcal{O}(\lvert P \rvert + \lvert \Sigma \rvert + \lvert O \rvert)$ 的常数空间内。只要状态模式的设计合理，系统就能实现真正的 $\mathcal{O}(1)$ 提示词开销，进而将整个任务执行周期的累积 Token 消耗锁定为纯粹的线性关系 $\mathcal{O}(T)$。
