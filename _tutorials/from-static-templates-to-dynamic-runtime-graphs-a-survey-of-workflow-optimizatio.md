...
1.  **ACG模板**（**ACG Template, $\bar{\mathcal{G}}$**）：这是一个可复用的设计蓝图。它定义了一个Agent系统所有可能的结构和参数空间，是离线优化的主要对象。

2.  **实现图**（**Realized Graph, $\mathcal{G}^{\mathrm{run}}$**）：针对某一次具体运行（run），实际被执行的工作流结构。它是从模板中实例化或动态生成的。

3.  **执行迹**（**Execution Trace, $\tau$**）：当实现图被执行后，产生的一系列状态、动作、观察和成本的序列记录。
...
1.  提出了**智能计算图**（**ACG**）作为统一的抽象，并区分了**模板**、**实现图**和**执行迹**。

2.  以“结构确定时间”为纲，清晰地划分了**静态优化**与**动态生成**两大技术路线。
