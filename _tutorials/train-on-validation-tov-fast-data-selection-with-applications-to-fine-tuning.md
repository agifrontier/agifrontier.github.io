...
本文旨在解决现有数据选择方法**计算成本高、实现复杂**的具体问题。它提出了一种更简单、更快速的替代方案，旨在不牺牲性能的前提下，大幅降低数据选择过程的计算开销。
...
2.  **聚合分数**: 使用一个聚合函数 $$F$$（如 $$F(y)=y$$, $$F(y)=\lvert y \rvert$$, $$F(y)=max{y,0}$$）处理每个token的分数后，再对整个序列求平均，得到该样本的最终ToV分数。
